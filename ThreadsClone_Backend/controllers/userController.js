...
const unfollowedUsers = async (req, res) => {
	try {
	  ...
	  if (!mongoose.Types.ObjectId.isValid(userId)) {
		...
		return res.status(400).json({ error: "Nevažeći ID formata" });
	  }
	  ...
	  const followersDetails = await User.find({ _id: { $in: currentUser.followers } });
	  const followingIds = currentUser.following.map(id => {
		try {
		  // Provera da li je ID već u formatu ObjectId
	  ...
	  const unfollowedUsers = followersDetails.filter(follower => !followingIds.includes(follower._id));
...
const getUserFollowing = async (req, res) => {
    try {
        ...
        console.log("Following users:", followingDetails);
